...
export default function Vendors() {
  ...
  const openEdit = (r) => { setSelected(r); setForm({ name: r.name, contact: r.contact || '', email: r.email || '', address: r.address || '', creditLimit: r.creditLimit || '' }); setModal(true) }
  ...
  return (
    <div>
      <div className="flex flex-wrap items-start sm:items-center justify-between gap-3 mb-4">
        <div><h1 className="text-xl font-bold text-gray-900">Vendors</h1><p className="text-xs text-slate-400 mt-0.5">{data.length} total</p></div>
        <button onClick={openCreate} className="btn-primary">+ Add Vendor</button>
      </div>
      <div className="table-wrap">
        <div className="filter-bar">
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search vendors" className="input flex-1 min-w-48" />
        </div>
        <table className="w-full text-sm">
          <thead className="table-head">
            <tr>{['Name', 'Contact', 'Email', 'Address', 'Credit Limit', 'Actions'].map(h => <th key={h} className="px-4 py-3.5 text-left">{h}</th>)}</tr>
          </thead>
              ...
              : paginated.length === 0 ? <tr><td colSpan={6} className="text-center py-8 text-slate-400">No results</td></tr>
                : paginated.map(r => (
                  <tr key={r.id} className="table-row">
                    ...
                    <td className="px-4 py-3.5 text-slate-600">{r.creditLimit ? `${Number(r.creditLimit).toLocaleString()}` : ''}</td>
      ...
      {confirmModal}
      {modal && (
        <Modal title={selected ? 'Edit Vendor' : 'Add Vendor'} onClose={() => setModal(false)}>
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div><label className="label">Credit Limit </label>
              <input type="number" value={form.creditLimit} onChange={set('creditLimit')} className="input" /></div>
