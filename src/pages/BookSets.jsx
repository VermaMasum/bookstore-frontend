...
export default function BookSets() {
  ...
  const [form, setForm] = useState({ name: '', schoolId: '', className: '', sessionYear: '' })
  const [bookItems, setBookItems] = useState({})

  const openCreate = () => { setSelected(null); setForm({ name: '', schoolId: '', className: '', sessionYear: '' }); setBookItems({}); setModal(true) }
  const openEdit = (r) => {
    setSelected(r)
    setForm({ name: r.name, schoolId: r.schoolId, className: r.className, sessionYear: r.sessionYear })
    const items = {}
    r.items?.forEach(i => { items[i.bookId] = { bookId: i.bookId, quantity: i.quantity } })
    setBookItems(items)
    setModal(true)
  }
  ...
  const getItemsForSubmit = () =>
    Object.values(bookItems).filter(i => parseInt(i.quantity) > 0).map(i => ({
      bookId: i.bookId, quantity: parseInt(i.quantity),
    }))
      ...
      <div className="table-wrap">
        ...
        <table className="w-full text-sm">
          <thead className="table-head">
            <tr>{['Set Name', 'School', 'Class', 'Session', 'Books', 'Total Price', 'Actions'].map(h => <th key={h} className="px-4 py-3.5 text-left">{h}</th>)}</tr>
          </thead>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input required value={form.className} onChange={e => setForm(f => ({ ...f, className: e.target.value }))} placeholder="e.g. Class 6" className="input" />
              </div>
              <div>
                <label className="label">Session Year *</label>
                <input required value={form.sessionYear} onChange={e => setForm(f => ({ ...f, sessionYear: e.target.value }))} placeholder="e.g. 2025-26" className="input" />
              </div>
            </div>
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="text-xs font-medium text-slate-600">Select Books for Set</label>
                {selectedCount > 0 && <span className="text-xs font-medium text-blue-600">{selectedCount} books in set  Price auto-calculated from MRP</span>}
              </div>
              <BookPickerTable books={bookList} items={Object.values(bookItems)} onChange={handleBookChange} checkboxMode />
            </div>
        ...
        <Modal title={viewModal.name} onClose={() => setViewModal(null)}>
          <div className="space-y-3 text-sm">
            ...
            <table className="w-full border-t">
              <thead><tr className="text-xs text-slate-500 uppercase">{['Book', 'Qty', 'MRP'].map(h => <th key={h} className="py-2 text-left">{h}</th>)}</tr></thead>
              <tbody className="divide-y divide-slate-100">
                {viewModal.items?.map(i => (
                  <tr key={i.id}><td className="py-1.5">{i.book?.title}</td><td className="py-1.5">{i.quantity}</td><td className="py-1.5">{i.book?.mrp}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
