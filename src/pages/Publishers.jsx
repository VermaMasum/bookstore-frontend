...
import { publishers as api } from '../api'
...
export default function Publishers() {
  ...
  const openEdit = (r) => { setSelected(r); setForm({ name: r.name, contact: r.contact || '', email: r.email || '', address: r.address || '' }); setModal(true) }
  ...
  const handleDelete = async (id) => {
    if (!await confirm('Delete this publisher?')) return
  ...
  return (
    <div>
      <div className="flex flex-wrap items-start sm:items-center justify-between gap-3 mb-4">
        <div><h1 className="text-xl font-bold text-gray-900">Publishers</h1><p className="text-xs text-slate-400 mt-0.5">{data.length} total</p></div>
        <button onClick={openCreate} className="btn-primary">+ Add Publisher</button>
      </div>

      <div className="table-wrap">
        <div className="filter-bar">
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by name, contact, email" className="input flex-1 min-w-48" />
        </div>
        <table className="w-full text-sm">
          <thead className="table-head">
            <tr>{['Name', 'Contact', 'Email', 'Address', 'Actions'].map(h => <th key={h} className="px-4 py-3.5 text-left">{h}</th>)}</tr>
          </thead>
          <tbody className="divide-y divide-gray-50">
            {loading ? <tr><td colSpan={5} className="text-center py-8 text-slate-400">Loading...</td></tr>
              : paginated.length === 0 ? <tr><td colSpan={5} className="text-center py-8 text-slate-400">{search ? 'No results' : 'No publishers yet'}</td></tr>
                : paginated.map(r => (
      ...
      {modal && (
        <Modal title={selected ? 'Edit Publisher' : 'Add Publisher'} onClose={() => setModal(false)}>
          <form onSubmit={handleSubmit} className="space-y-3">
            {[['name', 'Name *', true], ['contact', 'Contact', false], ['email', 'Email', false], ['address', 'Address', false]].map(([k, l, req]) => (
              <div key={k}>
                <label className="label">{l}</label>
                <input required={req} value={form[k]} onChange={set(k)} className="input" />
              </div>
            ))}
