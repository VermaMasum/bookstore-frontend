...
const empty = { name: '', type: 'LOCAL', contact: '', email: '', address: '' }
...
export default function Suppliers() {
  ...
  const { paginated, filtered, search, setSearch, filter, setFilter, page, setPage } = useTableState(data, {
    searchFields: ['name', 'contact', 'email'], filterField: 'type', perPage: 10,
  })
  ...
  const openEdit = (r) => { setSelected(r); setForm({ name: r.name, type: r.type, contact: r.contact || '', email: r.email || '', address: r.address || '' }); setModal(true) }
  ...
  return (
    <div>
      <div className="flex flex-wrap items-start sm:items-center justify-between gap-3 mb-4">
        <div><h1 className="text-xl font-bold text-gray-900">Suppliers</h1><p className="text-xs text-slate-400 mt-0.5">{data.length} total</p></div>
      ...
      <div className="table-wrap">
        ...
        <table className="w-full text-sm">
          <thead className="table-head">
            <tr>{['Name', 'Type', 'Contact', 'Email', 'Address', 'Actions'].map(h => <th key={h} className="px-4 py-3.5 text-left">{h}</th>)}</tr>
          </thead>
              ...
              : paginated.length === 0 ? <tr><td colSpan={6} className="text-center py-8 text-slate-400">No results</td></tr>
                : paginated.map(r => (
                  <tr key={r.id} className="table-row">
                    ...
                    <td className="px-4 py-3.5"><Badge value={r.type} /></td>
      ...
      {confirmModal}
      {modal && (
        <Modal title={selected ? 'Edit Supplier' : 'Add Supplier'} onClose={() => setModal(false)}>
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div><label className="label">Type</label>
              <select value={form.type} onChange={set('type')} className="select">
                <option value="LOCAL">Local</option><option value="EXTERNAL">External</option>
              </select></div>
